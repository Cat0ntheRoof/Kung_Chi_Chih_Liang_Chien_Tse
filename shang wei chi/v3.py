...
class AirQualityApp:
    def __init__(self, master):
        self.master = master
        master.title('空气质量监测系统 2021040714')
        ...
        # 添加分析文本框
        self.analysis_label = tk.Label(master, text='智能分析:', anchor='w')
        self.analysis_label.pack(side=tk.TOP, fill=tk.X, padx=10, pady=(5,0))
        
        self.analysis_text = tk.Text(master, height=6, state='disabled')
        ...
        self.export_btn = tk.Button(control_frame, text="导出数据", command=self.export_data)
    ...
    def update_display(self, param, value, is_warning):
        ...
        # 更新带时间戳的数据
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]  # 添加毫秒精度
